...
export async function downloadReport(params: ReportParams): Promise<void> {
  ...
  const [analyticsRes, billsRes, expensesRes] = await Promise.all([
    fetch(
      `${apiUrl}/api/analytics/${restaurantId}/restaurantDashboardOverview?${bParam}&range=custom&from=${from}&to=${to}`,
      { headers: h },
    ),
    fetch(
      `${apiUrl}/api/bills/${restaurantId}/restaurantwise?${bParam}&from=${from}&to=${to}`,
      { headers: h },
    ),
    fetch(`${apiUrl}/api/reports/expenses?${bParam}&from=${from}&to=${to}`, {
      headers: h,
    }),
  ]);
  ...
  const totalGST = bills.reduce(
    (s, b) => s + Number(b.cgst || 0) + Number(b.sgst || 0),
    0,
  );
  ...
  s1.columns = [{ width: 28 }, { width: 22 }, { width: 22 }, { width: 18 }];
  ...
  const kpis = [
    ...
    [
      "Paid Revenue",
      INR(paidBills.reduce((s, b) => s + Number(b.total || 0), 0)),
      `${paidBills.length} paid bills`,
    ],
    [
      "Total Orders",
      String(analytics.totalOrders || bills.length),
      "completed",
    ],
    [
      "Avg Bill Value",
    ...
    [
      "Unique Customers",
      String(analytics.totalCustomers || 0),
      "visited in period",
    ],
  ...
  const pnl = [
    ...
    [
      ...
      `−${INR(totalGST)}`,
      totalRevenue > 0
        ? `-${((totalGST / totalRevenue) * 100).toFixed(1)}%`
        : "0%",
    ],
    [
      "(−) Operating Expenses",
  ...
  pnl.forEach((r, i) => {
    ...
    if (i === pnl.length - 1) {
      ...
      pRow.fill = {
        type: "pattern",
        pattern: "solid",
        fgColor: { argb: netProfit >= 0 ? "FFD1FAE5" : "FFFEE2E2" },
      };
    }
  });
  ...
  const totalRow = s2.addRow([
    ...
    totalRevenue,
    bills.length,
    bills.length ? Math.round(totalRevenue / bills.length) : 0,
  ]);
  ...
  bills.forEach((b) => {
    (b.items || []).forEach((item: any) => {
      if (!itemMap[item.itemName])
        itemMap[item.itemName] = { qty: 0, revenue: 0 };
