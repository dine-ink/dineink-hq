...
function freezeAt(ws: ExcelJS.Worksheet, row: number) {
  ws.views = [
    { state: "frozen", xSplit: 0, ySplit: row, activeCell: `A${row + 1}` },
  ];
}
...
function drow(
  ...
  r.eachCell((cell, col) => {
    ...
    cell.alignment = {
      vertical: "middle",
      horizontal: rightCols.includes(col)
        ? "right"
        : col === 1
          ? "left"
          : "center",
...
export async function generateExcelReport(
  ...
  const avgCLV = customers.length
    ? Math.round(
        customers.reduce((s: number, c: any) => s + n(c.spend), 0) /
          customers.length,
      )
    : 0;
  ...
  const mappedItems = menuItems.filter(
    (m: any) => (m.menuItemIngredients || []).length > 0,
  );
  ...
  const inventoryStockValue = (data.ingredients || []).reduce(
    (sum: number, ing: any) => sum + n(ing.quantity) * n(ing.pricePerUnit),
    0,
  );
  const localFoodExp =
    n(ins.manualFoodCost) > 0
  ...
  const varExp = fin
    ...
    : n(ins.deliveryCharges) +
      n(ins.packaging) +
      n(ins.paymentGateway) +
      n(ins.aggregatorCommission) +
      n(ins.electricity) +
      n(ins.gas) +
      n(ins.maintenance) +
      n(ins.fuel);
  const labourExp = fin
    ? fin.labourCost
    : allStaff.reduce((s, st) => s + n(st.salary), 0);
  const finExp = fin
    ? fin.financeCost
    : n(ins.monthlyLoanEmi) +
      n(ins.monthlyInterestPayments) +
      n(ins.caFees) +
      n(ins.insuranceCost) +
      n(ins.otherTaxes);
  ...
  const ebitda = fin
    ? fin.ebitdaPercentage.toFixed(1)
    : insRev > 0
      ? (
          ((insRev - (fixedExp + varExp + labourExp + finExp + foodExp)) /
            insRev) *
          100
        ).toFixed(1)
      : "0";
  const primeCostPct = fin
    ? fin.primeCostPercentage.toFixed(1)
    : insRev > 0
      ? (((foodExp + labourExp) / insRev) * 100).toFixed(1)
      : "0";
  ...
  const targetGrossMarginVal =
    targets?.targetGrossMargin ?? n(ins.targetGrossMargin);
  const foodCostPct = fin
    ? fin.foodCostPercentage.toFixed(1)
    : insRev > 0
      ? ((foodExp / insRev) * 100).toFixed(1)
      : "0";
  const grossMarginPct = fin
    ? fin.grossProfitMarginPercentage.toFixed(1)
    : insRev > 0
      ? (((insRev - foodExp) / insRev) * 100).toFixed(1)
      : "0";
  const ebitdaAmount = fin
    ? fin.ebitda
    : insRev - fixedExp - varExp - labourExp - foodExp;

  // Expense by type
  const expByType: Record<string, number> = {};
  expenses.forEach((e) => {
    expByType[e.expenseType || "Other"] =
      (expByType[e.expenseType || "Other"] || 0) + n(e.amount);
  });
  ...
  inventoryAdjustments.forEach((a) => {
    ...
    wasteByIng[nm].qty += n(a.quantity);
    wasteByIng[nm].adj++;
  });
  ...
  // Accent strip
  const acr = cov.addRow([""]);
  acr.height = 4;
  for (let c = 1; c <= 5; c++) acr.getCell(c).fill = fill(T.brand);

  // Subtitle
  const subRow = cov.addRow([
    "27 Sheets  ·  Comprehensive Financial & Operational Data",
  ]);
  subRow.height = 16;
  ...
  // ════════════════════════════════════════════════════
  {
    ...
    title(
      ws,
      "Executive Summary",
      `${meta.restaurantName}  |  ${meta.branchName}  |  ${meta.from} to ${meta.to}`,
      5,
    );
    section(ws, "KEY PERFORMANCE INDICATORS", 5);
    const h = thead(
      ws,
      ["Metric", "Value", "Secondary Info", "Period", "Status"],
      T.brand,
      T.white,
      true,
    );
    ...
    const kpiRows = [
      ...
      [
        "Paid Revenue",
        inr(paidBills.reduce((s, b) => s + n(b.total), 0)),
      ...
      [
        "Total Orders",
        String(bills.length),
        `${analytics.totalOrders || bills.length} completed`,
        "Period",
        "OK",
      ],
      [
        "Avg Order Value",
        inr(bills.length ? Math.round(totalRev / bills.length) : 0),
        "per transaction",
        "Period",
        "—",
      ],
      ...
      [
        "Net Profit",
        inr(netProfit),
        `${totalRev > 0 ? ((netProfit / totalRev) * 100).toFixed(1) : 0}% margin`,
        "Period",
        netProfit >= 0 ? "OK" : "Loss",
      ],
      [
        ...
        `${customers.filter((c: any) => c.visits > 1).length} repeat`,
        "All time",
        "—",
      ],
      [
      ...
      [
        "At Risk Customers",
        String(atRiskC.length),
        "30-90 days since visit",
        "All time",
        "Warn",
      ],
      [
        "Churned Customers",
        String(churnedC.length),
        "90+ days away",
        "All time",
        "Loss",
      ],
      ...
      if (row[0] === "Net Profit") {
        r.eachCell((c) => {
          c.font = {
            bold: true,
            size: 9,
            color: { argb: netProfit >= 0 ? T.green : T.redDk },
            name: "Calibri",
          };
          c.fill = fill(netProfit >= 0 ? T.greenLt : T.lossRed);
        });
      }
    });
    ...
    const h2 = thead(
      ws,
      ["Line Item", "Amount (INR)", "% of Revenue", "Notes", ""],
      T.green,
      T.white,
      true,
    );
    ...
    srow(
      ...
      [
        ...
        "Revenue-GST-Expenses",
        "",
      ],
      netProfit >= 0 ? T.greenLt : T.lossRed,
      netProfit >= 0 ? T.greenDk : T.lossText,
    );
  }
  ...
  // ════════════════════════════════════════════════════
  {
    ...
    title(
      ...
      "Live operational metrics mirroring the main dashboard",
      6,
    );
    section(ws, "SUMMARY KPIs", 6);
    const h = thead(
      ws,
      ["KPI", "Value", "Orders", "Avg Value", "Customers", "Peak Hours"],
      T.brand,
      T.white,
      true,
    );
    ...
    dailyArr.forEach(([date, d], i) => {
      const ots = bills.filter(
        (b) => new Date(b.createdAt).toLocaleDateString("en-IN") === date,
      );
      const otStr = Object.entries(
        ots.reduce((a, b: any) => {
          a[b.orderType || "?"] = (a[b.orderType || "?"] || 0) + 1;
          return a;
        }, {} as any),
      )
        .map(([k, v]) => `${k.replace("_", " ")}: ${v}`)
        .join(", ");
      const pmStr = Object.entries(
        ots.reduce((a, b: any) => {
          a[b.paymentMethod || "?"] = (a[b.paymentMethod || "?"] || 0) + 1;
          return a;
        }, {} as any),
      )
        .map(([k, v]) => `${k}: ${v}`)
      ...
      drow(
        ...
        [
          ...
          d.rev,
          d.orders,
          d.orders ? Math.round(d.rev / d.orders) : 0,
          otStr,
          pmStr,
        ],
        i % 2 === 0,
      ...
      .forEach(([t, d], i) => {
        drow(
          ...
          [
            ...
            d.rev,
            pct(d.rev, totalRev),
            d.count ? Math.round(d.rev / d.count) : 0,
            "",
          ],
          i % 2 === 0,
          [2, 3, 5],
    ...
    thead(
      ws,
      ["#", "Item Name", "Qty Sold", "Revenue (INR)", "Category", "Sales %"],
      T.brand,
      T.white,
      true,
    );
    topItems.slice(0, 30).forEach(([name, d], i) => {
      drow(
        ws,
        [i + 1, name, d.qty, d.rev, d.cat, pct(d.rev, totalRev)],
        i % 2 === 0,
        [3, 4],
      );
    });
  ...
  // ════════════════════════════════════════════════════
  {
    ...
    const h = thead(
      ws,
      ["Date", "Revenue (INR)", "Orders", "Avg Bill (INR)", "YoY %", "Notes"],
      T.brand,
      T.white,
      true,
    );
    freezeAt(ws, h.number);
    dailyArr.forEach(([d, v], i) =>
      drow(
        ws,
        [
          d,
          v.rev,
          v.orders,
          v.orders ? Math.round(v.rev / v.orders) : 0,
          "—",
          "",
        ],
        i % 2 === 0,
    ...
    srow(
      ...
      [
        ...
        inr(totalRev),
        bills.length,
        bills.length ? Math.round(totalRev / bills.length) : 0,
        "",
        "",
      ],
      T.brandLt,
    ...
    const h2 = thead(
      ws,
      ["Hour", "Revenue (INR)", "Orders", "Avg Bill (INR)", "Revenue %", ""],
      T.brand,
      T.white,
      true,
    );
    freezeAt(ws, h2.number);
    const hourlyData = Object.entries(analytics.hourlyAnalytics || {}).sort(
    ...
    hourlyData.forEach(([h, d]: any, i) => {
      ...
      drow(
        ...
        [
          ...
          d.revenue,
          d.orders,
          d.orders ? Math.round(d.revenue / d.orders) : 0,
  ...
  // ════════════════════════════════════════════════════
  {
    const ws = newSheet(
      wb,
      "Billing Overview",
      [22, 14, 20, 16, 14, 12, 12, 12, 14, 12, 18],
    );
    title(
      ws,
      "Billing Overview",
    ...
    [
      ...
      ["Paid Bills", String(paidBills.length), T.green],
      ["GST Collected", inr(totalGST), T.amber],
    ].forEach(([l, v, c]) => kpi(ws, l as string, v as string, c as string));
    ...
    srow(
      ...
      [
        ...
        "",
        "",
        bills.reduce((s, b) => s + n(b.subtotal), 0),
        totalGST,
        totalDisc,
        totalRev,
  ...
  // ════════════════════════════════════════════════════
  {
    const ws = newSheet(
      wb,
      "Customer Overview",
      [24, 16, 12, 18, 16, 20, 16, 14],
    );
    title(
      ws,
      "Customer Overview",
      `${customers.length} customers  ·  all-time data`,
      8,
    );
    section(ws, "CUSTOMER KPIs", 5);
    [
      ["Total Customers", String(customers.length), T.blue],
      [
        "Repeat Customers",
        String(customers.filter((c: any) => c.visits > 1).length),
        T.green,
      ],
      ["Avg Spend", inr(avgCLV), T.purple],
      [
        "Total Revenue",
        inr(customers.reduce((s: number, c: any) => s + n(c.spend), 0)),
        T.brand,
      ],
    ].forEach(([l, v, c]) => kpi(ws, l as string, v as string, c as string));
      ...
      .forEach((c: any, i: number) => {
        ...
        const r = drow(
          ...
          [
            ...
            c.phone,
            c.visits,
            c.visits ? Math.round(n(c.spend) / c.visits) : 0,
            c.spend,
            c.preferredOrderType || "—",
            c.lastVisit
              ? new Date(c.lastVisit).toLocaleDateString("en-IN")
              : "—",
            rfmMap[c.phone]?.segment || seg,
          ],
          i % 2 === 0,
          [4, 5],
        );
        const segC: Record<string, string> = {
          Champion: T.green,
          Loyal: T.blue,
          Potential: T.purple,
          "At Risk": T.amber,
        ...
        sc.font = {
          bold: true,
          size: 8.5,
          color: { argb: segC[rfmMap[c.phone]?.segment || seg] || T.slate },
          name: "Calibri",
        };
      });
  }
  ...
  // ════════════════════════════════════════════════════
  {
    ...
    title(
      ...
      "Active  ·  At Risk  ·  Churned customer segmentation",
      7,
    );
    section(ws, "CHURN KPIs", 5);
    [
      ["Active (<=30 days)", String(activeC.length), T.green],
      ["At Risk (30-90 days)", String(atRiskC.length), T.amber],
      ["Churned (90+ days)", String(churnedC.length), T.redDk],
      ["Avg CLV", inr(avgCLV), T.purple],
    ].forEach(([l, v, c]) => kpi(ws, l as string, v as string, c as string));
    ...
    freezeAt(ws, h2.number);
    atRiskC
      .sort((a: any, b: any) => a.spend - b.spend)
      .forEach((c: any, i: number) => {
        const days = c.lastVisit
          ? Math.floor((now - new Date(c.lastVisit).getTime()) / 86400000)
          : 999;
        drow(
          ws,
          [
  ...
  // ════════════════════════════════════════════════════
  {
    const ws = newSheet(
      wb,
      "Customer RFM",
      [24, 16, 16, 10, 10, 10, 12, 16, 12, 16],
    );
    title(
      ws,
      "Customer RFM Analysis",
      "Recency  ·  Frequency  ·  Monetary scoring model",
      10,
    );
    ...
    [
      ...
      kpi(
        ws,
        `${seg} Customers`,
        `${segCounts[seg] || 0} customers  |  INR ${(rfm?.segmentRevenue?.[seg] || 0).toLocaleString("en-IN")} revenue`,
        c as string,
      );
    });
    section(ws, "RFM SCORING EXPLANATION", 6);
    const expRow = ws.addRow([
      "R = Recency (1-5): days since last visit  ·  F = Frequency (1-5): total visits  ·  M = Monetary (1-5): total spend  ·  Total 13-15 = Champion  ·  10-12 = Loyal  ·  7-9 = Potential  ·  5-6 = At Risk  ·  3-4 = Lost",
    ]);
    expRow.getCell(1).font = {
      size: 8.5,
      italic: true,
      color: { argb: T.slate },
      name: "Calibri",
    ...
    expRow.height = 14;
    ws.mergeCells(expRow.number, 1, expRow.number, 10);
    ws.addRow([]);
    ...
    (rfm?.customers || []).forEach((c: any, i: number) => {
      ...
      r.getCell(3).font = {
        bold: true,
        size: 8.5,
        color: { argb: segColors[c.segment] || T.slate },
        name: "Calibri",
      };
    });
  }
  ...
  // ════════════════════════════════════════════════════
  {
    ...
    title(
      ws,
      "Menu Analytics",
      "Ingredient intelligence, food cost and operational metrics",
      6,
    );
    section(ws, "MENU KPIs", 5);
    const invVal = (data.ingredients as any[]).reduce(
      (s, i) => s + n(i.quantity) * n(i.pricePerUnit),
      0,
    );
    [
      ["Inventory Value", inr(invVal), T.blue],
      [
        "Total Ingredients",
        String((data.ingredients as any[]).length),
        T.purple,
      ],
      ["Mapped Items", String(mappedItems.length), T.green],
      [
        "Unmapped Items",
        String(menuItems.length - mappedItems.length),
        T.amber,
      ],
    ].forEach(([l, v, c]) => kpi(ws, l as string, v as string, c as string));
    ...
    (data.ingredients as any[])
      .sort(
        (a, b) =>
          n(b.quantity) * n(b.pricePerUnit) - n(a.quantity) * n(a.pricePerUnit),
      )
      .forEach((ing: any, i: number) => {
        drow(
          ...
          [
            ...
            ing.unit || "—",
            ing.pricePerUnit || 0,
            (n(ing.quantity) * n(ing.pricePerUnit)).toFixed(2),
          ],
          i % 2 === 0,
          [3, 5, 6],
        );
    ...
    const h2 = thead(
      ws,
      ["Alert Type", "Item", "Detail", "Priority", "", ""],
      T.brand,
      T.white,
      true,
    );
    ...
    [
      ...
      [
        "Inventory Status",
        invVal > 0 ? "Healthy" : "Low Stock",
        `INR ${invVal.toLocaleString("en-IN")} value`,
        invVal > 0 ? "OK" : "WARN",
        "",
        "",
      ],
  ...
  // ════════════════════════════════════════════════════
  {
    ...
    const medianQty =
      itemsSales.length > 0
        ? itemsSales[Math.floor(itemsSales.length / 2)].qty
        : 0;
    const medianMargin =
      itemsSales.length > 0
        ? parseFloat(
    ...
    itemsSales.forEach((item, i) => {
      ...
      r.getCell(7).font = {
        bold: true,
        size: 8.5,
        color: { argb: quadColors[quad] || T.slate },
        name: "Calibri",
      };
    });
  }
  ...
  // ════════════════════════════════════════════════════
  {
    ...
    title(
      ws,
      "Insights Dashboard",
      "EBITDA  ·  Profitability  ·  Revenue targets  ·  Expense distribution",
      5,
    );
    section(ws, "INSIGHTS KPIs", 5);
    [
      ["Revenue (Monthly)", inr(insRev), T.brand],
      ["EBITDA", inr(ebitdaAmount), T.green],
      ["EBITDA %", `${ebitda}%`, T.purple],
      ["Prime Cost %", `${primeCostPct}%`, T.amber],
    ].forEach(([l, v, c]) => kpi(ws, l as string, v as string, c as string));
    ...
    [
      [
        ...
        `${ebitda}%`,
        `${targetEbitda || 0}%`,
        `${(parseFloat(ebitda) - (targetEbitda || 0)).toFixed(1)}%`,
        parseFloat(ebitda) >= (targetEbitda || 0) ? "Healthy" : "Critical",
      ],
      [
        "Food Cost %",
        `${foodCostPct}%`,
    ...
    [0, 5, 10, 15, 20, 25].forEach((target, i) => {
      const req =
        (fixedExp + varExp + labourExp + foodExp) / (1 - target / 100);
      ...
      if (done)
        r.eachCell((c) => {
          ...
          c.fill = fill(T.greenLt);
        });
    });
    section(ws, "EXPENSE DISTRIBUTION", 5);
    thead(
      ws,
      ["Expense Category", "Amount (INR)", "% of Total Expenses", "Notes", ""],
      T.brand,
      T.white,
      true,
    );
    ...
    [
      ...
      ["Finance & Tax", finExp, "Loans, CA fees, Insurance, Taxes"],
    ].forEach(([l, v, note], i) => {
      drow(
        ws,
        [l, inr(v), pct(v as number, totalAllExp), note, ""],
        i % 2 === 0,
        [2],
      );
    });
  ...
  // P&L Statement
  {
    ...
    const h = thead(
      ws,
      ["Line Item", "Amount (INR)", "% of Gross", "Notes", ""],
      T.green,
      T.white,
      true,
    );
    ...
    [
      ...
      [
        ...
        inr(bills.reduce((s, b) => s + n(b.serviceCharge), 0)),
        "—",
        "Service charges",
        "",
      ],
    ...
    const h2 = thead(
      ws,
      ["Tax Type", "Amount (INR)", "% of Net Revenue", "Notes", ""],
      T.amber,
      T.white,
      true,
    );
    ...
    [
      [
        "CGST",
        inr(bills.reduce((s, b) => s + n(b.cgst), 0)),
        pct(
          bills.reduce((s, b) => s + n(b.cgst), 0),
          totalRev,
        ),
        "Central GST",
        "",
      ...
      [
        "SGST",
        inr(bills.reduce((s, b) => s + n(b.sgst), 0)),
        pct(
          bills.reduce((s, b) => s + n(b.sgst), 0),
          totalRev,
        ),
        "State GST",
        "",
    ...
    const h3 = thead(
      ws,
      ["Expense Category", "Amount (INR)", "% of Revenue", "Entries", ""],
      T.purple,
      T.white,
      true,
    );
    ...
    Object.entries(expByType)
      ...
      .forEach(([type, amt], i) => {
        const cnt = expenses.filter(
          (e) => (e.expenseType || "Other") === type,
        ).length;
        drow(
          ws,
          [type, inr(amt), pct(amt, totalRev), String(cnt), ""],
          i % 2 === 0,
          [2],
        );
      });
  ...
  // Tax Report
  {
    ...
    title(
      ws,
      "Tax Report (GST)",
      `CGST + SGST breakdown  ·  ${meta.from} to ${meta.to}`,
      9,
    );
    ...
    [
      ["Total CGST", inr(cgstTotal), T.blue],
      ["Total SGST", inr(sgstTotal), T.purple],
      ["Total GST", inr(totalGST), T.brand],
      ["Taxable Revenue", inr(totalRev - totalGST), T.green],
    ].forEach(([l, v, c]) => kpi(ws, l as string, v as string, c as string));
  ...
  // Expense Tracker
  {
    ...
    [
      ["Total Expenses", inr(totalExp), T.purple],
      ["Expense Categories", String(Object.keys(expByType).length), T.blue],
      [
        "Avg per Entry",
        inr(expenses.length ? Math.round(totalExp / expenses.length) : 0),
        T.amber,
      ],
      [
        "Expense/Revenue Ratio",
        pct(totalExp, totalRev),
        totalExp / totalRev < 0.3 ? T.green : T.redDk,
      ],
    ].forEach(([l, v, c]) => kpi(ws, l as string, v as string, c as string));
    section(ws, "EXPENSE BY CATEGORY", 5, T.purple);
    const h1 = thead(
      ws,
      ["Category", "Amount (INR)", "% of Total", "Entries", ""],
      T.purple,
      T.white,
      true,
    );
      ...
      .forEach(([type, amt], i) => {
        drow(
          ...
          [
            ...
            pct(amt, totalExp),
            String(
              expenses.filter((e) => (e.expenseType || "Other") === type)
                .length,
            ),
            "",
          ],
    ...
    const h2 = thead(
      ws,
      ["Expense Name", "Category", "Date", "Amount (INR)", "Paid By"],
      T.purple,
      T.white,
      true,
    );
  ...
  // Sales Analytics
  {
    ...
    title(
      ws,
      "Sales Analytics",
      "Payment methods  ·  Order channels  ·  Bills summary",
      5,
    );
    section(ws, "PAYMENT METHOD REVENUE", 5, T.blue);
    const h1 = thead(
      ws,
      ["Payment Method", "Bills", "Revenue (INR)", "Avg Bill (INR)", "Share %"],
      T.blue,
      T.white,
      true,
    );
    ...
    const h2 = thead(
      ws,
      ["Channel", "Revenue (INR)", "Orders", "Avg Bill (INR)", "Revenue %"],
      T.blue,
      T.white,
      true,
    );
  ...
  // Discount Analysis
  {
    const ws = newSheet(
      wb,
      "Discount Analysis",
      [22, 14, 20, 16, 14, 14, 14, 12],
    );
    title(
      ws,
      "Discount Analysis",
      "Track every discount — identify patterns, prevent abuse",
      8,
    );
    section(ws, "DISCOUNT KPIs", 5);
    const discBills = bills.filter((b) => n(b.discount) > 0);
    [
      ["Total Discounts", inr(totalDisc), T.amber],
      ["Bills with Discount", String(discBills.length), T.blue],
      [
        "Avg Discount/Bill",
        inr(discBills.length ? Math.round(totalDisc / discBills.length) : 0),
        T.purple,
      ],
      [
        "Discount % of Revenue",
        pct(totalDisc, gross),
        totalDisc / gross < 0.05 ? T.green : T.redDk,
      ],
    ].forEach(([l, v, c]) => kpi(ws, l as string, v as string, c as string));
    ...
    freezeAt(ws, h.number);
    discBills
      .sort((a: any, b: any) => n(b.discount) - n(a.discount))
  ...
  // Table Analytics
  {
    ...
    title(
      ws,
      "Table Analytics",
      "Turn rate  ·  Revenue per table  ·  Occupancy",
      6,
    );
    ...
    [
      ...
      [
        "Total Orders",
        String(tableTurnData.reduce((s: number, t: any) => s + n(t.count), 0)),
        T.green,
      ],
    ].forEach(([l, v, c]) => kpi(ws, l as string, v as string, c as string));
    ...
    tableTurnData
      ...
      .forEach((t: any, i: number) => {
        const perf =
          t.count >
          tableTurnData.reduce((s: number, x: any) => s + x.count, 0) /
            Math.max(tableTurnData.length, 1)
            ? "High"
            : "Normal";
        const r = drow(
          ws,
          [t.name || t.tableName, t.count, t.avgTime, "—", "—", perf],
          i % 2 === 0,
        );
        if (perf === "High")
          r.getCell(6).font = {
  ...
  // Waste Report
  {
    ...
    title(
      ...
      `${inventoryAdjustments.length} adjustments logged`,
      6,
    );
    section(ws, "WASTE KPIs", 5);
    const wastage = inventoryAdjustments.filter(
      (a) => a.adjustmentType === "WASTAGE" || a.adjustmentType === "EXPIRED",
    );
    const damage = inventoryAdjustments.filter(
      (a) => a.adjustmentType === "DAMAGE",
    );
    [
      ["Total Adjustments", String(inventoryAdjustments.length), T.slate],
      ["Wastage/Expired", String(wastage.length), T.amber],
      ["Damage", String(damage.length), T.redDk],
      ["Unique Ingredients", String(Object.keys(wasteByIng).length), T.purple],
    ].forEach(([l, v, c]) => kpi(ws, l as string, v as string, c as string));
    ...
    inventoryAdjustments.forEach((a: any, i: number) => {
      ...
      r.getCell(3).font = {
        bold: true,
        size: 8.5,
        color: { argb: typeColors[a.adjustmentType] || T.slate },
        name: "Calibri",
      };
    });
  }
  ...
  // Hourly Heatmap
  {
    ...
    [
      ["Peak Hour", ph.label || "—", T.brand],
      ["Best Day", pd.name || "—", T.blue],
      ["Peak Hour Orders", String(ph.orders || 0), T.green],
      ["Best Day Orders", String(pd.orders || 0), T.purple],
    ].forEach(([l, v, c]) => kpi(ws, l as string, v as string, c as string));
    ...
    hd.filter((h: any) => h.hour >= 6).forEach((h: any, i: number) => {
      const r = drow(
        ...
        [
          ...
          h.revenue,
          h.orders,
          h.orders ? Math.round(h.revenue / h.orders) : 0,
          pct(
            h.revenue,
            hd.reduce((s: number, x: any) => s + x.revenue, 0),
          ),
          h.revenue === ph.revenue ? "Peak" : "",
        ],
        i % 2 === 0,
        [2, 4],
      );
      if (h.revenue === ph.revenue)
        r.eachCell((c) => {
          c.fill = fill(T.brandLt);
        });
    });
    ...
    dd.forEach((d: any, i: number) => {
      const r = drow(
        ...
        [
          ...
          pct(
            d.revenue,
            dd.reduce((s: number, x: any) => s + x.revenue, 0),
          ),
          d.revenue === pd.revenue ? "Best" : "",
        ],
        i % 2 === 0,
        [2, 4],
      );
      if (d.revenue === pd.revenue)
        r.eachCell((c) => {
          c.fill = fill(T.blueLt);
        });
    });
  }
  ...
  // Day Analysis
  {
    ...
    title(
      ...
      "Revenue, orders and performance by day",
      6,
    );
    const dd = heatmap?.dailyData || [];
    ...
    dd.forEach((d: any, i: number) => {
      ...
      const perf =
        d.revenue === 0
          ? "No Data"
          : d.revenue / maxDayRev >= 0.9
            ? "Peak Day"
            : d.revenue / maxDayRev < 0.5
              ? "Slow Day"
              : "Normal";
      const r = drow(
        ws,
      ...
      if (perf === "Peak Day")
        r.eachCell((c) => {
          ...
          c.font = {
            bold: true,
            size: 8.5,
            color: { argb: T.greenDk },
            name: "Calibri",
          };
        });
      else if (perf === "Slow Day")
        r.eachCell((c) => {
          c.fill = fill(T.amberLt);
        });
    });
  }
  ...
  // Revenue Forecast
  {
    ...
    title(
      ws,
      "Revenue Forecast",
      "7-day prediction  ·  Rolling average  ·  Growth trends",
      5,
    );
    const fSum = (forecast as any)?.summary || {};
    section(ws, "FORECAST KPIs", 5);
    [
      ["Last 7-Day Avg", inr(fSum.avg7 || 0), T.blue],
      ["Prev 7-Day Avg", inr(fSum.avgPrev7 || 0), T.slate],
      [
        "W-o-W Growth",
        `${fSum.growthPercent >= 0 ? "+" : ""}${fSum.growthPercent || 0}%`,
        (fSum.growthPercent || 0) >= 0 ? T.green : T.redDk,
      ],
      ["7-Day Forecast Total", inr(fSum.forecastTotal || 0), T.purple],
    ].forEach(([l, v, c]) => kpi(ws, l as string, v as string, c as string));
    section(ws, "30-DAY HISTORY", 5);
    const h1 = thead(
      ws,
      ["Date", "Actual Revenue (INR)", "", "", ""],
      T.brand,
      T.white,
      true,
    );
    freezeAt(ws, h1.number);
    ((forecast as any)?.history || []).forEach((f: any, i: number) =>
      drow(ws, [f.date, f.revenue, "", "", ""], i % 2 === 0, [2]),
    );
    ...
    ((forecast as any)?.forecast || []).forEach((f: any, i: number) => {
      drow(
        ws,
        [f.date, inr(f.predicted), inr(f.lower), inr(f.upper), "+-15%"],
        i % 2 === 0,
        [2, 3, 4],
      );
    });
  ...
  // Staff Attendance
  {
    const ws = newSheet(
      wb,
      "Staff Attendance",
      [24, 16, 16, 14, 12, 12, 12, 14, 14],
    );
    title(
      ws,
      "Staff Attendance",
      `${allStaff.length} staff  ·  Attendance register`,
      9,
    );
    const presentCount = new Set(
      attendance.filter((a) => a.loginTime).map((a) => a.userId),
    ).size;
    const lateCount = attendance.filter(
      (a) =>
        a.loginTime &&
        (new Date(a.loginTime).getHours() > 9 ||
          (new Date(a.loginTime).getHours() === 9 &&
            new Date(a.loginTime).getMinutes() > 15)),
    ).length;
    section(ws, "ATTENDANCE KPIs", 5);
    [
      ["Total Staff", String(allStaff.length), T.blue],
      ["Present", String(presentCount), T.green],
      ["Absent", String(allStaff.length - presentCount), T.redDk],
      ["Late Arrivals", String(lateCount), T.amber],
      [
        "Total Hours",
        `${attendance.reduce((s, a) => s + n(a.totalHours), 0).toFixed(0)}h`,
        T.purple,
      ],
    ].forEach(([l, v, c]) => kpi(ws, l as string, v as string, c as string));
    ...
    allStaff.forEach((s: any, i: number) => {
      ...
      const bm = (att?.breaks || []).reduce(
        (sum: number, b: any) => sum + n(b.totalMinutes),
        0,
      );
      const isPresent = !!att?.loginTime;
      const isLate =
        isPresent &&
        (new Date(att.loginTime).getHours() > 9 ||
          (new Date(att.loginTime).getHours() === 9 &&
            new Date(att.loginTime).getMinutes() > 15));
      const r = drow(
        ws,
        [
          s.name,
  ...
  // Staff Productivity
  {
    const ws = newSheet(
      wb,
      "Staff Productivity",
      [24, 16, 14, 14, 12, 14, 14, 14],
    );
    title(
      ws,
      "Staff Productivity",
      "Labour cost, shift efficiency and department breakdown",
      8,
    );
    const sp = staffProductivity || {};
    section(ws, "PRODUCTIVITY KPIs", 5);
    [
      ["Total Staff", String(sp.totals?.totalStaff || allStaff.length), T.blue],
      ["Hours Worked", `${sp.totals?.totalHoursWorked || 0}h`, T.amber],
      [
        "Monthly Labour Cost",
        inr(
    ...
    const totalShiftRev = Object.values(srev).reduce<number>(
      (s, v: any) => s + n(v),
      0,
    );
    [
      ["Morning (6-12 AM)", srev.morning],
      ["Afternoon (12-5 PM)", srev.afternoon],
      ["Evening (5-10 PM)", srev.evening],
      ["Night (10 PM-6 AM)", srev.night],
    ].forEach(([shift, rev], i) => {
      drow(
        ws,
        [shift, inr(rev), pct(n(rev), totalShiftRev), "", ""],
        i % 2 === 0,
        [2],
      );
    });
  ...
  // Cash Reconciliation
  {
    ...
    const shortfall = cashSessions.reduce(
      (s, r) => s + Math.abs(Math.min(0, n(r.cashDifference))),
      0,
    );
    const surplus = cashSessions.reduce(
      (s, r) => s + Math.max(0, n(r.cashDifference)),
      0,
    );
    section(ws, "CASH KPIs", 5);
    [
      ["Total Sessions", String(cashSessions.length), T.blue],
      ["Total Shortfall", inr(shortfall), T.redDk],
      ["Total Surplus", inr(surplus), T.green],
      [
        "Net Difference",
        inr(surplus - shortfall),
        surplus >= shortfall ? T.green : T.redDk,
      ],
    ].forEach(([l, v, c]) => kpi(ws, l as string, v as string, c as string));
      ...
      const r = drow(
        ...
        [
          ...
          s.openedAt
            ...
            : "—",
          s.closedAt
            ? new Date(s.closedAt).toLocaleTimeString("en-IN", {
                hour: "2-digit",
                minute: "2-digit",
              })
            : "—",
    ...
    srow(
      ...
      [
        ...
        "",
        "",
        cashSessions.reduce((s, r) => s + n(r.openingCash), 0),
        cashSessions.reduce((s, r) => s + n(r.expectedCash), 0),
        cashSessions.reduce((s, r) => s + n(r.actualCash || r.closingCash), 0),
        surplus - shortfall,
      ],
      T.greenLt,
      T.greenDk,
  ...
  // Kitchen Analytics
  {
    ...
    title(
      ...
      "Order speed  ·  SLA compliance  ·  Throughput",
      5,
      T.amber,
    );
    ...
    [
      ["Total Orders", String(ks.totalOrders || 0), T.blue],
      [
        "Avg Completion",
        `${ks.avgTime || 0} min`,
        ks.avgTime <= 30 ? T.green : T.amber,
      ],
      [
        "SLA Compliance",
        `${ks.slaPercent || 0}%`,
        (ks.slaPercent || 0) >= 80 ? T.green : T.redDk,
      ],
      ["Fastest Order", `${ks.fastestOrder || 0} min`, T.green],
      ["Peak Hour", ks.peakHourLabel || "—", T.amber],
    ].forEach(([l, v, c]) => kpi(ws, l as string, v as string, c as string));
      ...
      .forEach((h: any, i: number) => {
        drow(
          ...
          [
            ...
            h.orders,
            h.avgTime,
            h.avgTime <= 30 ? "Within SLA" : "Over SLA",
            "",
          ],
          i % 2 === 0,
        );
    ...
    const h2 = thead(
      ws,
      ["Order Type", "Count", "Avg Time (min)", "SLA Status", ""],
      T.amber,
      T.white,
      true,
    );
    ...
    (kitchenData?.orderTypeSpeeds || []).forEach((o: any, i: number) => {
      drow(
        ...
        [
          ...
          o.count,
          o.avgTime,
          o.avgTime <= 30 ? "Within SLA" : "Over SLA",
          "",
        ],
        i % 2 === 0,
      );
  ...
  // Branch Comparison
  {
    const ws = newSheet(
      wb,
      "Branch Comparison",
      [24, 18, 12, 16, 14, 16, 14, 12],
    );
    title(
      ws,
      "Branch vs Branch Comparison",
      `${branchComparison.length} branches analysed`,
      8,
      T.blue,
    );
    ...
    branchComparison.forEach((b: any, i: number) => {
      const r = drow(
        ...
        [
          ...
          b.revenue,
          b.orders,
          b.orders ? Math.round(b.revenue / b.orders) : 0,
          b.gst,
          b.netProfit,
          b.staffCount,
          b.revenue === maxBRev ? "Top" : "",
        ],
        i % 2 === 0,
        [2, 4, 5, 6],
      );
      if (b.revenue === maxBRev)
        r.eachCell((c) => {
          c.fill = fill(T.greenLt);
          c.font = {
            bold: true,
  ...
  // City Comparison
  {
    const ws = newSheet(
      wb,
      "City Comparison",
      [20, 14, 18, 14, 16, 16, 14, 12],
    );
    title(
      ws,
      "City vs City Comparison",
      `${cityComparison.length} cities analysed`,
      8,
      T.purple,
    );
    if (cityComparison.length > 1) {
      ...
      cityComparison.forEach((c: any, i: number) => {
        const r = drow(
          ...
          [
            ...
            c.revenue,
            c.orders,
            c.orders ? Math.round(c.revenue / c.orders) : 0,
            c.netProfit,
            c.staffCount || "—",
            c.revenue === maxCRev ? "Top" : "",
          ],
          i % 2 === 0,
          [3, 5, 6],
        );
        if (c.revenue === maxCRev)
          r.eachCell((c2) => {
            c2.fill = fill(T.purpleLt);
            c2.font = {
              bold: true,
              size: 8.5,
              color: { argb: T.purple },
      ...
      ws.addRow([]);
      const nr = ws.addRow([
        "Only one city found. Add branches with different cities in Settings to enable city comparison.",
      ]);
      nr.getCell(1).font = {
        italic: true,
        size: 10,
