...
export default function BusinessStats({analytics,}: Props) {
  const repeatCustomers = analytics?.totalCustomers ? Math.round(( analytics?.repeatCustomersCount / analytics?.totalCustomers) * 100,) : 0;
  const onlineRevenue = analytics?.recentOrders ?.filter((o: any) => o.orderType === "ONLINE",).reduce((sum: number, order: any,) => sum + order.total,0,) || 0;
  const dineInRevenue = analytics?.recentOrders ?.filter((o: any) => o.orderType === "DINE_IN",).reduce(( sum: number, order: any,) => sum + order.total,0,) || 0;
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        {stats.map((item) => {
          ...
          return (
            ...
            >
              {/* Soft Glow */}
              <div className={`absolute -right-8 -top-8 h-20 w-20 rounded-full blur-3xl ${item.glow}`}/>
