...
function getColorStyle({ variant, theme, color }: { variant: string; theme: any; color: string }) {
  ...
  switch (variant) {
    ...
    case 'light':
      return { color: main, background: lighter, '&:hover': { background: withAlpha(light, 0.5) }, ...commonShadow };
    case 'shadow':
      return { boxShadow: shadows, color: contrastText, background: main, '&:hover': { boxShadow: 'none', background: dark }, ...commonShadow };
    case 'outlined':
      return { '&:hover': { background: 'transparent', color: dark, borderColor: dark }, ...commonShadow };
    case 'dashed':
      return { background: lighter, '&:hover': { color: dark, borderColor: dark }, ...commonShadow };
    case 'text':
    default:
      return { '&:hover': { color: dark, background: color === 'secondary' ? withAlpha(light, 0.1) : lighter }, ...commonShadow };
  }
}
  ...
  ({ theme, color, variant }: any) => ({
    ...
    variants: [
      ...
      },
      {
        ...
        style: { border: '1px dashed', borderColor: 'inherit' }
      },
      {
